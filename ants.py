...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            if self.ant is not None and insect.can_contain(self.ant) is True:
                insect.place = self
                insect.contain_ant(self.ant)
                self.ant = insect
            if self.ant is not None and self.ant.can_contain(insect) is True:
                insect.place = self
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            if type(self.ant)==QueenAnt and self.ant.realqueen:
                return
            self.ant = None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        index=0
        place=self.place
        while place.entrance!=hive and index<=self.max_range:
            if place.bees!=[] and index>=self.min_range:
...
class BodyguardAnt(Ant):
    ...
    def reduce_armor(self, amount):
        if self.ant is not None and (self.armor - amount) <= 0:
            a = self.place
            Insect.reduce_armor(self, amount)
            a.ant = self.ant
        else:
...
class QueenAnt(ThrowerAnt):
    ...
    def __init__(self):
        ...
        self.changed_ants=[]
        if QueenAnt.num_instances==0:
            self.realqueen=True
        else:
            self.realqueen=False
        QueenAnt.num_instances+=1
    ...
    def action(self, colony):
        ...
        if not self.realqueen:
            ...
            self.throw_at(self.nearest_bee(colony.hive))
            colony.queen=QueenPlace(self, colony)
            myplace=self.place.exit
            while myplace.name!='AntQueen':
                if myplace.ant:
                    if myplace.ant not in self.changed_ants:
                            myplace.ant.damage = 2 * type(myplace.ant).damage
                            self.changed_ants.append(myplace.ant)
...
class QueenPlace(object):
    def __init__(self, queen, colony):
        self.queen=queen.place
        self.colony=colony.queen
    @property
    def bees(self):
        return self.colony.bees + self.queen.bees 
...
def make_slow(action):
    ...
    """
    def newslowaction(colony):
        if (colony.time % 2) == 0:
            action(colony)
        else:
            return None
    return newslowaction
...
def make_stun(action):
    ...
    action -- An action method of some Bee
    """
    def newstunaction(colony):
        return None
    return newstunaction
